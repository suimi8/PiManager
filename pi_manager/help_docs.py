...
3. 点 **拉取上游模型** → 用搜索框过滤（如 `qwen`、`:free`）→ **勾选要接入的模型** → 保存。不会默认写入全部上游模型。拉取返回 HTTP 503 / `upstream_overloaded` 时，只要已用模板预填模型即可直接保存，不必等拉取成功。  
4. 回到 **模型列表** → **刷新模型列表**。
...
- **从 API 拉取模型**：BaseURL + API Key 获取上游目录；可搜索、勾选后再保存，不会默认把全部模型写入。
