...
A：为同一 Provider 添加多把 Key 后，快速提问、HTTP 模型测试、模型列表拉取和 Cursor `pi.askPrompt` 会自动尝试下一把可用 Key。失败 Key 会保留在失效池；在 **Provider → API Keys** 中可恢复单个或全部 Key。只有明确的 Key 错误会进入失效池，服务端 5xx 或网络故障不会误停用。
...
解决：升级 Cursor 扩展到与本版本配套的 VSIX（Release 中的 `pi-manager-pi-cursor-*.vsix`）。
...
等字符时此前可经 Windows `cmd.exe` shim 注入任意命令（校验全平台生效）。请把
